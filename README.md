...
The HLS suite (`pnpm test:e2e e2e/hls.spec.ts`) generates a real H.264/PCM MKV fixture that requires compatibility playback on both browsers. It checks manifest and segment delivery, forward/backward seeks beyond the generated window with correct absolute playback time, and prompt FFmpeg process termination when leaving the player or replacing a seek window. The API, media, database and cwd-relative caches are isolated from development data. Interrupted-network recovery is not covered yet.
