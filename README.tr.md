...
E2E paketi Chromium ve WebKit üzerinde gerçek video süresinin ilerlemesini, ileri/geri sarmayı ve sunucuya kaydedilen konumdan sayfa yenilendikten sonra devam etmeyi doğrular. İki tarayıcıyı `pnpm exec playwright install --with-deps chromium webkit` ile kurun; yalnız birini çalıştırmak için `pnpm test:e2e --project=chromium` veya `--project=webkit` kullanın. CI, Chromium için Linux ve WebKit için macOS kullanır; zorunlu `e2e` kontrolü ancak iki paket de başarılıysa geçer. Medya codec desteği platforma göre değişir: [Playwright WebKit](https://playwright.dev/docs/browsers#webkit), gerçek Safari veya fiziksel iOS cihazı testi değildir. Safari'ye en yakın kapsam için WebKit oynatma kontrollerini macOS üzerinde çalıştırın.

HLS paketi (`pnpm test:e2e e2e/hls.spec.ts`), iki tarayıcıda da uyumluluk oynatması gerektiren gerçek bir H.264/PCM MKV dosyası üretir. Manifest ve segment aktarımını, üretilmiş akış aralığının dışına ileri/geri sararken mutlak oynatma konumunun doğruluğunu ve oynatıcıdan çıkıldığında veya sarma aralığı değiştirildiğinde FFmpeg işleminin kısa sürede sonlanmasını doğrular. Teste özel yerel bir proxy, WebKit'in yerel medya istekleri dahil gerçek HLS isteklerine HTTP 503 yanıtları verir: başarısız segment, uzun kesinti, konumun korunması, toparlanma sırasında duraklatma, sınırlı deneme ve manuel yeniden deneme sınanır. Bunlar kontrollü aktarım hatalarıdır; gerçek Wi-Fi/hücresel ağ geçişi testleri değildir. API, medya, veritabanı ve çalışma dizinine göre oluşturulan önbellekler geliştirme verilerinden izoledir.

HLS toparlanması uygulama düzeyinde en fazla üç kez, 1, 2 ve 4 saniye aralıklarla yeniden dener; toparlanma başladıktan sonra toplam süre sınırı 30 saniyedir. Oynatılabilir tampon verisi kalmayan bir takılma 12 saniye sonra algılanır. Geçerli akış konumu ve oynatma tercihi korunur; duraklatılmış video duraklatılmış kalır. Denemeler tükenirse bağlantı geldikten sonra **Akışı Tekrar Dene** düğmesini kullanın. Deneme sayacı istikrarlı oynatma, kaynak değişimi veya açık bir manuel yeniden deneme ile sıfırlanır. Doğrudan video ve müzik oynatımı ayrı kurtarma yolları kullanır.
