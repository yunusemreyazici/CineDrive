...
HLS paketi (`pnpm test:e2e e2e/hls.spec.ts`), iki tarayıcıda da uyumluluk oynatması gerektiren gerçek bir H.264/PCM MKV dosyası üretir. Manifest ve segment aktarımını, üretilmiş akış aralığının dışına ileri/geri sararken mutlak oynatma konumunun doğruluğunu ve oynatıcıdan çıkıldığında veya sarma aralığı değiştirildiğinde FFmpeg işleminin kısa sürede sonlanmasını doğrular. API, medya, veritabanı ve çalışma dizinine göre oluşturulan önbellekler geliştirme verilerinden izoledir. Ağ kesintisinden toparlanma henüz bu kapsamda değildir.
