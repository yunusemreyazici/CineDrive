...
export const HomeMediaCard: React.FC<HomeMediaCardProps> = ({ media }) => {
  ...
  const imageUrl = getPosterUrl(media);
  ...
  return (
    <article className="group relative min-w-0">
      {/* The card navigates, but it also carries its own buttons, so the link is
          stretched across it instead of wrapping the interactive children. */}
      <Link
        to={`/media/${media.id}`}
      ...
      >
        <span className="sr-only">{t.mediaCard.openDetails(formatMediaTitle(media.title))}</span>
      </Link>
      <div className="overflow-hidden rounded-xl border border-white/[0.08] bg-[#111214] shadow-[0_14px_32px_rgba(0,0,0,0.18)] transition duration-300 group-hover:scale-[1.02] group-hover:bg-[#181a1d] group-hover:shadow-[0_18px_42px_rgba(0,0,0,0.32)]">
        ...
        <button
          ...
          className="absolute bottom-2.5 right-2.5 z-20 flex h-8 w-8 translate-y-2 items-center justify-center rounded-full border border-white/35 bg-black/60 text-white opacity-0 shadow-lg shadow-black/30 backdrop-blur transition duration-200 hover:border-brand-400 hover:bg-brand-600 group-hover:translate-y-0 group-hover:opacity-100 focus-visible:translate-y-0 focus-visible:opacity-100"
        >
        ...
        <button
          ...
          aria-label={
            media.isFavorite
              ? t.mediaCard.removeFavorite(formatMediaTitle(media.title))
              : t.mediaCard.addFavorite(formatMediaTitle(media.title))
          }
          aria-pressed={Boolean(media.isFavorite)}
          onClick={handleFavorite}
          className={`absolute right-2.5 top-2.5 z-20 flex h-7 w-7 items-center justify-center rounded-full border backdrop-blur-md transition ${
            media.isFavorite
        ...
        <div className="px-3 py-2.5">
          ...
          <p className="mt-0.5 flex items-center gap-2 text-[11px] text-zinc-500">
            ...
            <span>{media.type === 'movie' ? t.common.movie : t.common.series}</span>
          </p>
        </div>
      </div>
    </article>
