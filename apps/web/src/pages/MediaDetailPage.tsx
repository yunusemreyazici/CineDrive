...
import {
  ...
  useAutoDownloadSubtitleMutation,
  useMediaListQuery,
...
export const MediaDetailPage: React.FC = () => {
  ...
  const seasons = media?.series?.seasons || [];
  const currentSeason = selectedSeasonId
    ? seasons.find((season) => season.id === selectedSeasonId) || seasons[0]
    : seasons[0];
  const episodesQuery = useSeasonEpisodesQuery(currentSeason?.id);
  ...
  if (isError) {
    return (
      <ErrorState error={error} title={t.errors.contentLoadFailed} onRetry={() => void refetch()} />
    );
  }
    ...
    <div className="space-y-10">
          ...
          */}
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={`${SECONDARY_ACTION_CLASSES} ${media.isFavorite ? 'text-rose-400' : ''}`}
              title={media.isFavorite ? t.mediaDetail.favoriteRemove : t.mediaDetail.favoriteAdd}
              aria-label={
                media.isFavorite ? t.mediaDetail.favoriteRemove : t.mediaDetail.favoriteAdd
              }
            >
        ...
        <div className="space-y-6 pt-4">
            ...
            <div className="flex items-center gap-2">
              {seasons.map((season) => (
                <button
                  ...
                  aria-pressed={currentSeason?.id === season.id}
          ...
          <div className="space-y-4">
            {episodesQuery.isLoading ? (
              ...
              episodesQuery.data?.map((episode: EpisodeType) => {
      ...
      >
        <p className="p-6 text-sm leading-relaxed text-zinc-300">
          <strong className="text-white">{media.title}</strong> {t.mediaDetail.deleteConfirmPrefix}{' '}
