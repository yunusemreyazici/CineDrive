...
export const MusicReplayPage: React.FC = () => {
  ...
  const query = useMusicReplayQuery(
    period,
  ...
  const downloadCard = () => {
    ...
    data.genres.slice(0, 5).forEach((genre, index) => {
      ...
      context.fillRect(
        80,
        820 + index * 70,
        Math.max(90, (genre.seconds / (data.genres[0]?.seconds || 1)) * 820),
        38,
      );
      context.fillStyle = '#ffffff';
  ...
  if (query.isLoading)
    return <div className="h-[520px] animate-pulse rounded-[34px] bg-zinc-900" />;
  if (query.isError)
    return (
    ...
    <div className="space-y-9 pb-32">
      <header className="relative isolate overflow-hidden rounded-[36px] border border-white/10 bg-gradient-to-br from-cyan-950 via-violet-950 to-black p-7 md:p-12">
        ...
        <div className="flex flex-wrap items-end justify-between gap-8">
          <div>
            <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[.25em] text-cyan-300">
              <Sparkles className="h-4 w-4" /> CineDrive Music
            </p>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-black text-black"
          >
        ...
        <div className="mt-10 inline-flex rounded-full border border-white/10 bg-black/25 p-1">
          {(['week', 'month', 'year'] as const).map((item) => (
            <button
              ...
              className={`rounded-full px-5 py-2 text-xs font-bold ${period === item ? 'bg-white text-black' : 'text-white/50'}`}
            >
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        {stats.map(({ icon: Icon, label, value }) => (
          <div key={label} className="rounded-3xl border border-white/[.07] bg-white/[.035] p-6">
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <div className="rounded-[30px] border border-white/[.07] bg-white/[.025] p-6">
          <h2 className="mb-5 font-display text-2xl font-bold">{t.music.topArtists}</h2>
          <div className="space-y-3">
            {data.topArtists.slice(0, 6).map((artist, index) => (
              <div
                key={artist.id}
                className="flex items-center gap-4 rounded-2xl bg-white/[.035] p-3"
              >
                <span className="w-6 text-lg font-black text-white/25">{index + 1}</span>
                {artist.artworkUrl ? (
                  <img
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate font-bold">{artist.name}</p>
                  <p className="text-xs text-white/40">{formatDuration(artist.seconds)}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-[30px] border border-white/[.07] bg-white/[.025] p-6">
          <h2 className="mb-5 font-display text-2xl font-bold">{t.music.topAlbums}</h2>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {data.topAlbums.slice(0, 6).map((album) => (
              <div key={album.id} className="min-w-0">
                ...
                <p className="mt-2 truncate text-sm font-bold">{album.title}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="rounded-[30px] border border-white/[.07] bg-white/[.025] p-6">
        <h2 className="mb-5 font-display text-2xl font-bold">{t.music.listeningHours}</h2>
        <div className="flex h-44 items-end gap-1">
          {data.hours.map((hour) => (
            <div
              key={hour.hour}
              title={`${hour.hour}:00 · ${formatDuration(hour.seconds)}`}
              className="min-w-0 flex-1 rounded-t bg-gradient-to-t from-violet-600 to-cyan-300 transition hover:brightness-125"
              style={{ height: `${Math.max(3, (hour.seconds / maxHour) * 100)}%` }}
            />
          ))}
        </div>
      ...
      <section className="rounded-[30px] border border-white/[.07] bg-white/[.025] p-6">
        <h2 className="mb-5 font-display text-2xl font-bold">{t.music.listeningDays}</h2>
        <div className="grid grid-cols-7 gap-2">
          {data.weekdays.map((day) => (
            <div key={day.day} className="text-center">
              <div className="flex h-32 items-end rounded-xl bg-white/[.025] p-1">
                <div
                  className="w-full rounded-lg bg-gradient-to-t from-fuchsia-600 to-cyan-300"
                  style={{ height: `${Math.max(4, (day.seconds / maxDay) * 100)}%` }}
                />
              </div>
              <p className="mt-2 text-[10px] font-bold text-white/45">
                {new Intl.DateTimeFormat(document.documentElement.lang, {
                  weekday: 'short',
                }).format(new Date(2024, 0, 7 + day.day))}
              </p>
            </div>
          ))}
        </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.4fr_1fr]">
        <div className="rounded-[30px] border border-white/[.07] bg-white/[.025] p-6">
          <h2 className="mb-4 font-display text-2xl font-bold">Top Tracks</h2>
          <MusicTrackList tracks={data.topTracks.map((item) => item.track)} />
        </div>
        <div className="rounded-[30px] border border-white/[.07] bg-white/[.025] p-6">
          <h2 className="mb-5 flex items-center gap-2 font-display text-2xl font-bold">
            <BarChart3 className="h-5 w-5" /> {t.music.genreDistribution}
          </h2>
            ...
            {data.genres.map((genre) => (
              <div key={genre.name}>
                <div className="mb-1 flex justify-between text-xs">
                  ...
                  <span className="text-white/35">{formatDuration(genre.seconds)}</span>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-white/5">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-violet-500"
                    style={{ width: `${(genre.seconds / (data.genres[0]?.seconds || 1)) * 100}%` }}
                  />
                </div>
              </div>
            ))}
