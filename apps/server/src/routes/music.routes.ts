...
import {
  ...
  musicArtworkThumbnails,
  nearestMusicArtworkVariant,
} from '../services/music-artwork-thumbnail.service.js';
...
const downloadGrantKey = createHmac('sha256', env.SESSION_SECRET)
  .update('music-download-grant-v1')
  .digest();

type DownloadGrant = { u: string; t: string; f: 'original' | 'aac'; e: number };

const issueDownloadGrant = (userId: string, trackId: string, format: 'original' | 'aac') => {
  const encoded = Buffer.from(
    JSON.stringify({ u: userId, t: trackId, f: format, e: Date.now() + DOWNLOAD_GRANT_TTL_MS }),
  ).toString('base64url');
  const signature = createHmac('sha256', downloadGrantKey).update(encoded).digest('base64url');
  return `${encoded}.${signature}`;
};

const verifyDownloadGrant = (token: string): DownloadGrant | null => {
  ...
  const expected = Buffer.from(
    createHmac('sha256', downloadGrantKey).update(encoded).digest('base64url'),
  );
  const provided = Buffer.from(signature);
  if (expected.length !== provided.length || !timingSafeEqual(expected, provided)) return null;
  try {
    const grant = JSON.parse(Buffer.from(encoded, 'base64url').toString('utf8')) as DownloadGrant;
    if (!grant.u || !grant.t || !['original', 'aac'].includes(grant.f) || grant.e <= Date.now())
      return null;
    return grant;
  } catch {
    return null;
...
const manageableTrackWhere = (userId: string): Prisma.MusicTrackWhereInput => ({
  library: {
    OR: [{ userId }, { memberships: { some: { userId, role: { in: ['owner', 'editor'] } } } }],
  },
  driveFile: { status: 'active' },
});
...
export const musicRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  const beginDirectTransfer = (
    ...
    if (
      active.size >= (clientId === 'legacy' ? 4 : MAX_ACTIVE_DIRECT_MUSIC_TRANSFERS_PER_CLIENT) ||
  ...
  fastify.addHook('preHandler', async (request, reply) => {
    if (!request.user && (request.method === 'GET' || request.method === 'HEAD')) {
      const parsedURL = new URL(request.url, 'http://cinedrive.local');
      const match = parsedURL.pathname.match(/\/tracks\/([^/]+)\/download$/);
      const token = parsedURL.searchParams.get('downloadGrant');
      if (match?.[1] && token) {
  ...
  fastify.get('/sync', async (request, reply) => {
    ...
    const isV2 = parsed.data.version === 2;
    ...
    const albumWhere: Prisma.MusicAlbumWhereInput = {
      tracks: { some: trackWhere },
      ...(isV2 && cursor ? { updatedAt: { gt: cursor, lte: nextCursor } } : {}),
    };
    const artistWhere: Prisma.MusicArtistWhereInput = {
      trackCredits: { some: { track: trackWhere } },
      ...(isV2 && cursor ? { updatedAt: { gt: cursor, lte: nextCursor } } : {}),
    };
    const playlistWhere: Prisma.MusicPlaylistWhereInput = {
      userId,
      ...(isV2 && cursor ? { updatedAt: { gt: cursor, lte: nextCursor } } : {}),
    };
      ...
      isV2 && cursor
        ? fastify.prisma.musicTrack.findMany({
            where: {
              ...
              driveFile: {
                status: { not: 'active' },
                updatedAt: { gt: cursor, lte: nextCursor },
              },
            },
            select: { id: true },
          })
  ...
  fastify.patch<{ Querystring: { clientId?: string; clientName?: string; platform?: string } }>(
    '/playback-state',
    async (request, reply) => {
      const client = musicPlaybackClientQuerySchema.safeParse(request.query);
      if (!client.success)
  ...
  fastify.get<{
    ...
    if (request.query.variant && !isMusicArtworkVariant(request.query.variant)) {
      return reply.status(400).send({
        error: {
          code: 'INVALID_ARTWORK_VARIANT',
          message: 'Geçersiz kapak görseli boyutu.',
          requestId: request.id,
        },
      });
    }
    ...
    const variant =
      request.query.variant && isMusicArtworkVariant(request.query.variant)
        ? request.query.variant
        : nearestMusicArtworkVariant({ width: requestedWidth, height: requestedHeight });
    const responseVariant = shouldResize ? variant : 'original';
    const etag = `"${artwork.checksum}-${responseVariant}"`;
    if (request.headers['if-none-match'] === etag)
      return reply.header('ETag', etag).status(304).send();
    const source = Buffer.from(artwork.data);
    const thumbnail = shouldResize
      ? await musicArtworkThumbnails.thumbnail(artwork.checksum, source, variant)
      : null;
    reply
  ...
  fastify.post('/download-manifest', async (request, reply) => {
    ...
    return {
      ...
      items: parsed.data.trackIds.map((trackId) => {
        ...
        const downloadGrant = issueDownloadGrant(userId, trackId, parsed.data.format);
        return {
          trackId,
          url: `/api/music/tracks/${trackId}/download?format=${parsed.data.format}&downloadGrant=${encodeURIComponent(downloadGrant)}`,
          fileName:
  ...
  const handleTrackDownload = async (
    ...
    const requestedClientId = request.headers['x-cinemusic-client-id'];
    const transferClientId =
      typeof requestedClientId === 'string' && /^[a-zA-Z0-9_-]{6,128}$/.test(requestedClientId)
        ? requestedClientId
        : 'download';
    const transfer = beginDirectTransfer(userId, transferClientId, request, reply);
  ...
  const handleTrackStream = async (
    ...
    const userId = request.user!.id;
    const client = musicPlaybackClientQuerySchema.shape.clientId.safeParse(
      request.query.clientId || 'legacy',
    );
    if (!client.success)
      return reply.status(400).send({
        error: {
    ...
    if (!transfer) {
      fastify.log.warn(
        { userId, clientId: client.data, code: 'MUSIC_TRANSFER_CAPACITY_REACHED' },
        'Music transfer capacity reached',
      );
  ...
  fastify.get<{
    Params: { id: string };
    Querystring: { transcode?: string; start?: string; clientId?: string };
  }>('/tracks/:id/stream', (request, reply) => handleTrackStream(request, reply));
  fastify.head<{
    Params: { id: string };
    Querystring: { transcode?: string; start?: string; clientId?: string };
  }>('/tracks/:id/stream', (request, reply) => handleTrackStream(request, reply, true));
};
