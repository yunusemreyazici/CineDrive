...
export const audioQuality = (track: {
  id: string;
  audio?: { lossless?: boolean | null; bitDepth?: number | null; sampleRate?: number | null; bitrate?: number | null; codec?: string | null };
...
export class MusicMaintenanceService {
  ...
  public async generate(userId: string, input: { trackIds?: string[]; albumIds?: string[] }) {
    const tracks = await this.prisma.musicTrack.findMany({
      where: { library: { userId }, driveFile: { status: 'active' }, ...(input.trackIds?.length ? { id: { in: input.trackIds } } : {}) },
    ...
    const albumIds = [...new Set([...requestedAlbumIds, ...tracks.filter((track) => !track.album?.artworkId).map((track) => track.albumId).filter((id): id is string => !!id)])].slice(0, 20);
    let generated = 0;
    for (const track of tracks) {
      if (track.metadataLocked || !track.primaryArtist?.name) continue;
      const [recording, album] = await Promise.all([
        this.musicbrainz.enrichRecording({ recordingId: track.musicbrainzRecordingId, title: track.title, artist: track.primaryArtist.name, album: track.album?.title, duration: track.duration }),
        track.album ? this.musicbrainz.enrichAlbum(track.album.artist?.name || track.primaryArtist.name, track.album.title) : Promise.resolve(null),
      ]);
      ...
      const currentData = {
        ...
        credits: track.credits.filter((credit) => credit.source === 'musicbrainz').map((credit) => ({
          name: credit.name,
          role: credit.role,
      ...
      const proposedData = {
        ...
        genres: track.album?.genres && track.album.genres !== '[]' ? track.album.genres : JSON.stringify(album?.genres || []),
      ...
      const exists = await this.prisma.musicMaintenanceSuggestion.findFirst({ where: { userId, targetType: 'track', targetId: track.id, kind: 'metadata', status: 'pending' } });
      if (!exists) {
        await this.prisma.musicMaintenanceSuggestion.create({ data: { userId, targetType: 'track', targetId: track.id, kind: 'metadata', provider: 'musicbrainz', confidence: 90, currentData: json(currentData), proposedData: json(proposedData) } });
        generated += 1;
      }
    }
    const albums = await this.prisma.musicAlbum.findMany({ where: { userId, id: { in: albumIds }, artworkId: null }, include: { artist: true } });
    for (const album of albums) {
      ...
      const exists = await this.prisma.musicMaintenanceSuggestion.findFirst({ where: { userId, targetType: 'album', targetId: album.id, kind: 'artwork', status: 'pending' } });
      if (!exists) {
        await this.prisma.musicMaintenanceSuggestion.create({ data: {
          userId, targetType: 'album', targetId: album.id, kind: 'artwork', provider: 'cover-art-archive', confidence: 90,
          currentData: json({ artworkId: album.artworkId }),
          proposedData: json({ releaseGroupId: metadata.releaseGroupId, previewUrl: `https://coverartarchive.org/release-group/${metadata.releaseGroupId}/front-500` }),
        } });
        generated += 1;
      }
    }
  ...
  public async resolve(userId: string, suggestionId: string, accept: boolean) {
    const suggestion = await this.prisma.musicMaintenanceSuggestion.findFirst({ where: { id: suggestionId, userId, status: 'pending' } });
    if (!suggestion) return null;
    if (!accept) {
      await this.prisma.musicMaintenanceSuggestion.update({ where: { id: suggestion.id }, data: { status: 'rejected', resolvedAt: new Date() } });
    ...
    if (suggestion.kind === 'acoustic-metadata' && suggestion.targetType === 'track') {
      ...
      if (!track) return null;
      await this.prisma.$transaction(async (tx) => {
        await tx.musicTrack.update({ where: { id: track.id }, data: { musicbrainzRecordingId: (proposed.musicbrainzRecordingId as string | undefined) || undefined } });
        if (track.albumId) await tx.musicAlbum.update({ where: { id: track.albumId }, data: {
          year: (proposed.year as number | undefined) || undefined,
          genres: (proposed.genres as string | undefined) || undefined,
          musicbrainzReleaseId: (proposed.musicbrainzReleaseId as string | undefined) || undefined,
          musicbrainzReleaseGroupId: (proposed.musicbrainzReleaseGroupId as string | undefined) || undefined,
          metadataStatus: 'enriched',
        } });
        const credits = Array.isArray(proposed.credits) ? proposed.credits as Array<{ name: string; role: string; instrument?: string; musicbrainzId?: string }> : [];
        if (credits.length) {
          await tx.musicTrackCredit.deleteMany({ where: { trackId: track.id, source: 'musicbrainz' } });
          await tx.musicTrackCredit.createMany({ data: credits.map((credit, position) => ({ trackId: track.id, ...credit, instrument: credit.instrument || '', source: 'musicbrainz', position })) });
        }
        await tx.musicMaintenanceAction.create({ data: { userId, actionType: 'accept-metadata', targetType: 'track', targetId: track.id, beforeData: suggestion.currentData, afterData: suggestion.proposedData } });
        await tx.musicMaintenanceSuggestion.update({ where: { id: suggestion.id }, data: { status: 'accepted', resolvedAt: new Date() } });
      });
    } else if (suggestion.kind === 'artwork' && suggestion.targetType === 'album') {
      const album = await this.prisma.musicAlbum.findFirst({ where: { id: suggestion.targetId, userId } });
      ...
      await this.prisma.$transaction([
        ...
        this.prisma.musicMaintenanceAction.create({ data: { userId, actionType: 'accept-artwork', targetType: 'album', targetId: album.id, beforeData: json({ artworkId: album.artworkId }), afterData: json({ artworkId }) } }),
        this.prisma.musicMaintenanceSuggestion.update({ where: { id: suggestion.id }, data: { status: 'accepted', resolvedAt: new Date() } }),
      ]);
    }
    return { status: 'accepted' };
  }

  public async archiveDuplicate(userId: string, input: { keepTrackId: string; archiveTrackId: string; replacePlaylistItems: boolean }) {
    if (input.keepTrackId === input.archiveTrackId) return null;
    const tracks = await this.prisma.musicTrack.findMany({ where: { id: { in: [input.keepTrackId, input.archiveTrackId] }, library: { userId } }, include: { driveFile: true } });
    if (tracks.length !== 2) return null;
    const archived = tracks.find((track) => track.id === input.archiveTrackId)!;
    const playlistItems = input.replacePlaylistItems ? await this.prisma.musicPlaylistItem.findMany({ where: { trackId: archived.id, playlist: { userId } }, select: { id: true, trackId: true } }) : [];
    await this.prisma.$transaction(async (tx) => {
      await tx.driveFile.update({ where: { id: archived.driveFileId }, data: { status: 'archived' } });
      for (const item of playlistItems) await tx.musicPlaylistItem.update({ where: { id: item.id }, data: { trackId: input.keepTrackId } });
      await tx.musicMaintenanceAction.create({ data: {
        userId, actionType: 'archive-duplicate', targetType: 'track', targetId: archived.id,
        beforeData: json({ driveFileId: archived.driveFileId, status: archived.driveFile.status, playlistItems }),
  ...
  public async undo(userId: string, actionId: string) {
    const action = await this.prisma.musicMaintenanceAction.findFirst({ where: { id: actionId, userId, revertedAt: null } });
    if (!action) return null;
    const before = parseJson(action.beforeData);
    await this.prisma.$transaction(async (tx) => {
      if (action.actionType === 'archive-duplicate') {
        await tx.driveFile.update({ where: { id: before.driveFileId as string }, data: { status: (before.status as string) || 'active' } });
        for (const item of (before.playlistItems as Array<{ id: string; trackId: string }> | undefined) || []) await tx.musicPlaylistItem.update({ where: { id: item.id }, data: { trackId: item.trackId } });
      } else if (action.actionType === 'accept-artwork') {
        await tx.musicAlbum.update({ where: { id: action.targetId }, data: { artworkId: (before.artworkId as string | null) || null } });
      } else if (action.actionType === 'accept-metadata') {
        await tx.musicTrack.update({ where: { id: action.targetId }, data: { musicbrainzRecordingId: (before.musicbrainzRecordingId as string | null) || null } });
        ...
        if (track?.albumId) await tx.musicAlbum.update({ where: { id: track.albumId }, data: {
          ...
          musicbrainzReleaseGroupId: (before.musicbrainzReleaseGroupId as string | null) || null,
        } });
