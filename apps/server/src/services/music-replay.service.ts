...
export class MusicReplayService {
  ...
  public async get(userId: string, period: ReplayPeriod, year?: number) {
    ...
    const end = period === 'year' ? new Date(Date.UTC((year || new Date().getUTCFullYear()) + 1, 0, 1)) : new Date();
    ...
    const trackStats = new Map<string, { track: (typeof entries)[number]['track']; seconds: number; plays: number }>();
    const albumStats = new Map<string, { id: string; title: string; artworkUrl: string | null; seconds: number; plays: number }>();
    const artistStats = new Map<string, { id: string; name: string; artworkUrl: string | null; seconds: number; plays: number }>();
    ...
    for (const entry of entries) {
      ...
      const track = increment(trackStats, entry.trackId, () => ({ track: entry.track, seconds: 0, plays: 0 }));
      ...
      if (entry.track.album) {
        const album = increment(albumStats, entry.track.album.id, () => ({
          ...
          artworkUrl: entry.track.album!.artwork ? `/api/music/artwork/${entry.track.album!.artwork!.id}` : null,
          seconds: 0,
          plays: 0,
        }));
      ...
      for (const genre of parseGenres(entry.track.genres)) genres.set(genre, (genres.get(genre) || 0) + seconds);
    ...
    const bySeconds = <T extends { seconds: number }>(values: Iterable<T>) => [...values].sort((a, b) => b.seconds - a.seconds);
    return {
      period,
      ...
      topTracks: bySeconds(trackStats.values()).slice(0, 10).map((item) => ({ track: formatMusicTrack(item.track), seconds: item.seconds, plays: item.plays })),
      topAlbums: bySeconds(albumStats.values()).slice(0, 10),
      topArtists: bySeconds(artistStats.values()).slice(0, 10),
      hours,
      weekdays,
      genres: [...genres.entries()].map(([name, seconds]) => ({ name, seconds })).sort((a, b) => b.seconds - a.seconds).slice(0, 12),
    };
  }
}
