...
Bağımlılık taraması, on dakikalık adım sınırıyla `pnpm audit:ci` üzerinden bağımsız çalışır. Önce sınırlı istek tekrarlarıyla yalnızca üretim bağımlılıkları için npm audit çalıştırılır. npm'in high/critical bulguları, bozuk çıktı, yapılandırma/kimlik doğrulama hatası veya yarıda kesilen süreç fallback olmadan kapıyı engeller. Yalnızca tanınan geçici ağ hataları ve audit endpoint'inin geçici yanıtları (408/429/500/502/503/504) OSV-Scanner'ı devreye alabilir.

Fallback, [OSV-Scanner'ı](https://google.github.io/osv-scanner/supported-languages-and-lockfiles/) sabit sürümde indirir ve çalıştırmadan önce sabitlenmiş SHA-256 değerini doğrular. Geliştirme ve isteğe bağlı/platform paketleri dahil standart pnpm v9 lockfile'ının **tamamını** tarar; kapsamı ana üretim taramasından geniştir, kapsam ve açık verisi birebir aynı değildir. Kilitli her paket adı/sürümü JSON raporunda bulunmalıdır. CVSS 7.0 ve üzeri engellenir; bilinmeyen severity, eksik paket, boş/bozuk rapor, checksum uyuşmazlığı ve tarayıcı/ağ hataları da engellenir. Paket/açık istisnaları veya erişilebilirlik gerekçeli eleme kullanılmaz. Raporlar, stderr ve sağlayıcı kararı `dependency-audit-reports` artifact'ında yedi gün saklanır. Başarılı fallback açıkça OSV olarak raporlanır; npm audit başarılı olmuş gibi gösterilmez.

Audit servisi kesintileri uygulama kontrollerini iş bağımlılıkları üzerinden durdurmaz; paket kurulumu yine registry erişimi veya önbellek gerektirir. Mevcut zorunlu `e2e` kontrolü doğrulama, iki tarayıcı ve son audit kararını birleştirir. Başarısız, iptal edilmiş veya atlanmış audit kapıyı kırmızı tutar. `pnpm ci:test`, fallback politikasını, paket kapsamını, workflow bağımlılıklarını ve önceki iş sonuçlarının 125 birleşimini sınar. `pnpm audit:prod`, değiştirilmemiş npm-only tarama için kullanılabilir. OSV sürümünü ve asset digest'lerini `scripts/audit-dependencies.mjs` içinde birlikte inceleyip güncelleyin; desteklenmeyen lockfile biçimleri incelenene kadar engellenir.
